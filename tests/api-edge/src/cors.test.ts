...
const stageEnv: Env = { ENVIRONMENT: "stage", CONSOLE_CUSTOM_DOMAIN: "stage.fiscally.dev" };
const prodEnv: Env = { ENVIRONMENT: "prod", CONSOLE_CUSTOM_DOMAIN: "prod.fiscally.dev" };
...
describe("api-edge cors", () => {
  describe("isAllowedOrigin — stage environment", () => {
    it("allows the stage custom domain origin", () => {
      expect(isAllowedOrigin("https://stage.fiscally.dev", stageEnv)).toBe(true);
    });
    ...
    it("rejects the prod custom domain origin", () => {
      expect(isAllowedOrigin("https://prod.fiscally.dev", stageEnv)).toBe(false);
    });
    ...
    it("rejects the legacy Pages console origins", () => {
      expect(isAllowedOrigin("https://fiscally-web-console.pages.dev", stageEnv)).toBe(false);
      expect(isAllowedOrigin("https://fiscally-web-console-stage.pages.dev", stageEnv)).toBe(false);
      expect(isAllowedOrigin("https://fiscally-web-console-prod.pages.dev", stageEnv)).toBe(false);
      expect(isAllowedOrigin("https://abc123.fiscally-web-console-stage.pages.dev", stageEnv)).toBe(false);
    });
  ...
  describe("isAllowedOrigin — prod environment", () => {
    it("allows the prod custom domain origin", () => {
      expect(isAllowedOrigin("https://prod.fiscally.dev", prodEnv)).toBe(true);
    });
    ...
    it("rejects the stage custom domain origin", () => {
      expect(isAllowedOrigin("https://stage.fiscally.dev", prodEnv)).toBe(false);
    });
    ...
    it("rejects the legacy Pages console origins", () => {
      expect(isAllowedOrigin("https://fiscally-web-console.pages.dev", prodEnv)).toBe(false);
      expect(isAllowedOrigin("https://fiscally-web-console-prod.pages.dev", prodEnv)).toBe(false);
      expect(isAllowedOrigin("https://feat-branch-42.fiscally-web-console-stage.pages.dev", prodEnv)).toBe(false);
    });
  ...
  describe("isAllowedOrigin — fallback (test/unknown environment)", () => {
    it("rejects custom domain origins when CONSOLE_CUSTOM_DOMAIN is not set", () => {
      expect(isAllowedOrigin("https://stage.fiscally.dev", testEnv)).toBe(false);
      expect(isAllowedOrigin("https://prod.fiscally.dev", testEnv)).toBe(false);
    });
    ...
    it("rejects legacy Pages console origins in fallback env", () => {
      expect(isAllowedOrigin("https://fiscally-web-console-stage.pages.dev", testEnv)).toBe(false);
      expect(isAllowedOrigin("https://fiscally-web-console-prod.pages.dev", testEnv)).toBe(false);
    });
  ...
  describe("handlePreflight", () => {
    ...
    it("returns 204 with CORS headers for allowed stage custom domain", () => {
      const req = new Request("https://api.test/v1/auth/session", {
        method: "OPTIONS",
        headers: { origin: "https://stage.fiscally.dev" },
      });
      const res = handlePreflight(req, stageEnv);
      expect(res).not.toBeNull();
      expect(res!.status).toBe(204);
      expect(res!.headers.get("access-control-allow-origin")).toBe(
        "https://stage.fiscally.dev",
      );
    ...
    it("returns 204 without CORS headers when prod custom domain hits stage API", () => {
      const req = new Request("https://api.test/v1/auth/session", {
        method: "OPTIONS",
        headers: { origin: "https://prod.fiscally.dev" },
      });
      const res = handlePreflight(req, stageEnv);
      expect(res).not.toBeNull();
      expect(res!.status).toBe(204);
