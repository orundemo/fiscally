...
- **`cloudflare_hyperdrive_config`**: Hyperdrive gateway resource
  - **Name**: `{namespacePrefix}fiscally-{environment}` (e.g., `fiscally-stage`)
...
| `orgName` | string | `fiscally` | Organization name (for tags and secret paths) |
| `owner` | string | `fiscally` | Repository owner (for tags) |
| `repo` | string | `fiscally` | Repository name (for tags and secret paths) |
| `namespace` | string | `fiscally` | Namespace (for labels and naming conventions) |
...
1. **`supabase` component** (same repo)
   ...
   - This component reads credentials from the secret path: `sourceplane/fiscally/supabase/{environment}`
...
- **Secret path**: `sourceplane/fiscally/supabase/{environment}`
...
aws secretsmanager get-secret-value --secret-id sourceplane/fiscally/supabase/stage
```
