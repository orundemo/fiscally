...
`fiscally` organization and stores generated credentials in AWS Secrets
Manager.
...
|-------------|----------------------------|--------------|
| stage       | fiscally-stage    | Provisioned  |
| prod        | fiscally-prod     | Provisioned  |
| dev         | —                          | Not provisioned |
...
State is stored at:
```
s3://sourceplane-<env>/env/<env>/fiscally/supabase/terraform.tfstate
```
...
```
sourceplane/fiscally/supabase/stage
sourceplane/fiscally/supabase/prod
```
