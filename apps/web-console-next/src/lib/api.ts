...
// deleted; all wire I/O now goes through the typed `Fiscally` resource
...
//   - `createClient` — constructs a `Fiscally` against a target, wired
...
//     `Promise<T>` + thrown `FiscallyError` into the same shape the
...
import { Fiscally, FiscallyError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Fiscally;
...
export function createClient(target: ApiTarget, token: string | null): Fiscally {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Fiscally(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `FiscallyError` subclasses surface as `{ ok: false }` carrying the
