...
Your backend holds one Fiscally API key and **zero GitHub credentials**.
...
const fiscally = new Fiscally({
  baseUrl: process.env.FISCALLY_API_URL!,
  token: process.env.FISCALLY_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await fiscally.integrations.issueGithubToken(ORG_ID, {
