...
Part of the fiscally runtime: a Cloudflare Worker deployed per
environment (`stage`, `prod`; `dev` is verify-only). Public entry point: `https://fiscally-api-edge-{stage,prod}.orundemo.workers.dev`
...
- **web-console-next** — Next.js 15 + opennextjs/cloudflare delivery of the Fiscally web console (per-environment, Workers + Static Assets)
