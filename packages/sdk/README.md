...
Fiscally TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Fiscally control plane API.

## Install (workspace)

This package is internal to the `fiscally` monorepo. Add it as a
...
```ts
import { Fiscally } from "@saas/sdk";

const client = new Fiscally({
  baseUrl: "https://api.fiscally.dev",
  auth: { kind: "bearer", token: process.env["FISCALLY_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `FiscallyError`. Branch on
...
| `FiscallyError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `FiscallyError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Fiscally({ fetch })`.
