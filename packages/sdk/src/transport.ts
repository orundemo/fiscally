// Base HTTP transport for the Fiscally SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `FiscallyError`
...
export interface ClientOptions {
  /**
   * Base URL of the Fiscally api-edge worker, e.g.
   * `https://api.fiscally.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Fiscally` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Fiscally SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Fiscally SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
