// Typed error hierarchy for the Fiscally SDK.
//
...
// codes (forward compatibility) fall back to the generic `FiscallyError`
...
export interface FiscallyErrorInit {
 ...
 * `quota_exceeded`) decode to this class — `instanceof FiscallyError`
 * still matches.
 */
export class FiscallyError extends Error {
  ...
  constructor(init: FiscallyErrorInit) {
    super(init.envelope.message);
    this.name = "FiscallyError";
...
export class BadRequestError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class UnauthenticatedError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class ForbiddenError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class NotFoundError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class ConflictError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class ValidationError extends FiscallyError {
...
export class PreconditionFailedError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class UnsupportedError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export class InternalError extends FiscallyError {
  constructor(init: FiscallyErrorInit) {
    super(init);
...
export interface RateLimitErrorInit extends FiscallyErrorInit {
...
export class RateLimitError extends FiscallyError {
...
/**
 * Decode an HTTP `Response` into the appropriate `FiscallyError` subclass.
 *
 * Forward-compatible: unknown error codes resolve to the base `FiscallyError`.
...
export async function decodeError(
  ...
  switch (envelope.code as ErrorCode) {
    ...
    default:
      ...
      return new FiscallyError(init);
  }
}

function decodeRateLimit(
  init: FiscallyErrorInit,
  response: Response,
): RateLimitError {
