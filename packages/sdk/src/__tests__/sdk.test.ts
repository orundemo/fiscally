...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Fiscally({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => FiscallyError]> = [
  ...
  it("forward-compat: unknown error code → base FiscallyError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(FiscallyError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as FiscallyError).code).toBe("future_quota_exceeded");
    }
  });
