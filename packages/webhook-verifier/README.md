...
Fiscally outbound webhook deliveries. Runs verbatim on Cloudflare
...
export async function handler(request: Request): Promise<Response> {
  ...
  const result = await verifyWebhookSignature({
    secret: process.env.FISCALLY_WEBHOOK_SECRET!,
    body,
...
produces the exact `sha256=<hex>` header value Fiscally emits — useful
