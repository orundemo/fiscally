...
`fiscally` — first-class TypeScript CLI for the Fiscally control
...
This package is internal to the `fiscally` monorepo and is not
...
```
fiscally login    [--api-url=URL] [--token=BEARER]
fiscally logout
fiscally whoami
```
...
```
fiscally org list
fiscally org use <org-id>
fiscally org members
fiscally project list
```

Writes (Task 0101):

```
fiscally org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
fiscally project create <name> [--idempotency-key=KEY]
fiscally env create <project-id> <name> [--idempotency-key=KEY]
fiscally api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
fiscally webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
fiscally usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
fiscally billing summary
fiscally audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`fiscally org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `fiscally login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/fiscally/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/fiscally/config.json` (mode 0644, not a secret). Override
both via `FISCALLY_CONFIG_DIR` (used by tests).
