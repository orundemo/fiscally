// `fiscally webhook deliveries <endpointId> [--limit=N] [--cursor=CURSOR] [--all]`
...
export async function webhookDeliveriesCommand(
  ...
  if (endpointId === undefined || endpointId.length === 0) {
    throw new UsageError(
      "usage: fiscally webhook deliveries <endpointId> [--limit=N] [--cursor=CURSOR] [--all] [--output=human|json]",
    );
  }
