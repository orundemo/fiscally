// CLI error helpers. Translate `FiscallyError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `fiscally org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `fiscally login` to refresh", mode);
  }
  if (err instanceof FiscallyError) {
