...
describe("resolveConfigDir", () => {
  it("respects FISCALLY_CONFIG_DIR override", () => {
    const prev = process.env["FISCALLY_CONFIG_DIR"];
    process.env["FISCALLY_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["FISCALLY_CONFIG_DIR"];
      else process.env["FISCALLY_CONFIG_DIR"] = prev;
    }
  });
});
